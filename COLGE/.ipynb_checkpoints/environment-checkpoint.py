...
class Environment:
    ...
    def get_reward(self, observation, node):
        ...
        # https://en.wikipedia.org/wiki/Dominating_set
        elif self.name == "DOM_SET":
             ### Penalize each new vertex which is added
            new_nbr_nodes=np.sum(observation[0].numpy())
            ...
            dom_set = np.where(self.observation[0,:,0].numpy() == 1)[0]
            total_g = self.graph_init.get_graph()
            done = is_dominating_set(total_g, dom_set)
    ...
    def get_approx(self):
        ...
        if self.approx_sol[self.games] is None:
            ...
            if self.name=="MVC":
                ...
                self.approx_sol[self.games] = len(min_weighted_dominating_set(self.graph_init.g))
                return len(min_weighted_dominating_set(self.graph_init.g))
            elif self.name=="MIS":
    ...
    def get_optimal_sol(self):
        ...
        if self.opt_sol[self.games] is None:
            ...
            if self.name =="MVC":
                ...
                # LP problem
                mdl = pulp.LpProblem("DOM_SET", pulp.LpMinimize)

                # Objective function: size of dominating set
                mdl += sum(xv[k] for k in xv)
                    
                # Constraint: Every node must have a neighbor
                # or be in the dominating set
                for node in range(self.graph_init.g.number_of_nodes()):
                    adj = xv[node]
                    for neighbor in self.graph_init.g.neighbors(node):
                        adj += xv[neighbor]
                    mdl += (adj >= 1)
                
                # Find and return size of optimal (min) dominating set
                mdl.solve()
