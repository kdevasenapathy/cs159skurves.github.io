...
for nstep in nstep_candidates:
    filepath = ("runs/FeedbackVertex/optimizer_n_step" + str(nstep))
    start_time = time.time()
    %run main.py --model S2V_QN_1 --path filepath --environment_name MFVS --graph_type barabasi_albert --m 0.4 --node nodes --ngames 100 --n_step nstep --lr best_lrate --gamma best_g --lr_decay best_decay --bs batch_size --epoch max_epochs
    end_time = time.time()
    print("feedback vertex nstep = ", nstep, "time = ", end_time - start_time)
...
for lrate, decay in gridsearch_params:
    filepath = ("runs/FeedbackVertex/optimizer_lrate_" + str(lrate) + "_decay_" + str(decay))
    start_time = time.time()
    %run main.py --model S2V_QN_1 --path filepath --environment_name MFVS --graph_type barabasi_albert --m 0.4 --node nodes --ngames 100 --n_step best_nstep --lr lrate --gamma best_g --lr_decay decay --bs batch_size --epoch max_epochs
    end_time = time.time()
    print("feedback vertex lrate = ", lrate, ", decay = ", decay, "time = ", end_time - start_time)
    epoch, min_conv_cand = best_epoch(filepath, max_epochs)
    if min_conv_cand <= min_conv:
        best_lrate = lrate
        best_decay = decay
...
for g in g_candidates:
    filepath = ("runs/FeedbackVertex/optimizer_g_" + str(g))
    start_time = time.time()
    %run main.py --model S2V_QN_1 --path filepath --environment_name MFVS --graph_type barabasi_albert --m 0.4 --node nodes --ngames 100 --n_step best_nstep --lr best_lrate --gamma g --lr_decay best_decay --bs batch_size --epoch max_epochs
    end_time = time.time()
    print("feedback vertex gamma = ", g, "time = ", end_time - start_time)
    epoch, min_conv_cand = best_epoch(filepath, max_epochs)
    if min_conv_cand <= min_conv:
        best_g = g
        
# Get final number of epochs
filepath = ("runs/FeedbackVertex/optimizer_g_" + str(best_g))
epoch, min_conv_cand = best_epoch(filepath, max_epochs)
print("Feedback Vertex Cover: ")
print("Epochs: ", epoch)
