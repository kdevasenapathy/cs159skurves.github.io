...
for nstep in nstep_candidates:
    filepath = ("runs/MaxClique/optimizer_n_step" + str(nstep))
    start_time = time.time()
    %run main.py --model S2V_QN_1 --path filepath --environment_name MAX_CLIQUE --graph_type barabasi_albert --m 0.4 --node nodes --ngames 50 --n_step nstep --lr best_lrate --gamma best_g --lr_decay best_decay --bs batch_size --epoch max_epochs
    end_time = time.time()
    print("Max Clique nstep = ", nstep, "time = ", end_time - start_time)
...
decay_candidates = [0.5, 0.55, 0.6, 0.65, 0.7]
...
for lrate, decay in gridsearch_params:
    filepath = ("runs/MaxClique/optimizer_lrate_" + str(lrate) + "_decay_" + str(decay))
    start_time = time.time()
    %run main.py --model S2V_QN_1 --path filepath --environment_name MAX_CLIQUE --graph_type barabasi_albert --m 0.4 --node nodes --ngames 50 --n_step best_nstep --lr lrate --gamma best_g --lr_decay decay --bs batch_size --epoch max_epochs
    end_time = time.time()
    print("Max Clique lrate = ", lrate, ", decay = ", decay, "time = ", end_time - start_time)
    epoch, min_conv_cand = best_epoch(filepath, max_epochs)
    if min_conv_cand <= min_conv:
        best_lrate = lrate
        best_decay = decay
...
for g in g_candidates:
    filepath = ("runs/MaxClique/optimizer_g_" + str(g))
    start_time = time.time()
    %run main.py --model S2V_QN_1 --path filepath --environment_name MAX_CLIQUE --graph_type barabasi_albert --m 0.4 --node nodes --ngames 50 --n_step best_nstep --lr best_lrate --gamma g --lr_decay best_decay --bs batch_size --epoch max_epochs
    end_time = time.time()
    print("Max Clique gamma = ", g, "time = ", end_time - start_time)
    epoch, min_conv_cand = best_epoch(filepath, max_epochs)
    if min_conv_cand <= min_conv:
        best_g = g
        
# Get final number of epochs
filepath = ("runs/MaxClique/optimizer_g_" + str(best_g))
epoch, min_conv_cand = best_epoch(filepath, max_epochs)
