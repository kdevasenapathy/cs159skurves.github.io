...
for nstep in nstep_candidates:
    filepath = ("runs/DomSet/optimizer_n_step" + str(nstep))
    start_time = time.time()
    %run main.py --model S2V_QN_1 --path filepath --environment_name DOM_SET --graph_type erdos_renyi --node nodes --ngames 100 --n_step nstep --lr best_lrate --gamma best_g --lr_decay best_decay --bs batch_size --epoch max_epochs
    end_time = time.time()
    print("Dominating Set nstep = ", nstep, "time = ", end_time - start_time)
...
decay_candidates = [0.75, 0.8, 0.85, 0.9, 0.95, 0.9995]
...
for lrate, decay in gridsearch_params:
    filepath = ("runs/DomSet/optimizer_lrate_" + str(lrate) + "_decay_" + str(decay))
    start_time = time.time()
    %run main.py --model S2V_QN_1 --path filepath --environment_name DOM_SET --graph_type erdos_renyi --node nodes --ngames 100 --n_step best_nstep --lr lrate --gamma best_g --lr_decay decay --bs batch_size --epoch max_epochs
    end_time = time.time()
    print("Dominating Set lrate = ", lrate, ", decay = ", decay, "time = ", end_time - start_time)
    epoch, min_conv_cand = best_epoch(filepath, max_epochs)
    if min_conv_cand <= min_conv:
        best_lrate = lrate
        best_decay = decay
...
for g in g_candidates:
    filepath = ("runs/DomSet/optimizer_g_" + str(g))
    start_time = time.time()
    %run main.py --model S2V_QN_1 --path filepath --environment_name DOM_SET --graph_type erdos_renyi --node nodes --ngames 100 --n_step best_nstep --lr best_lrate --gamma g --lr_decay best_decay --bs batch_size --epoch max_epochs
    end_time = time.time()
    print("Dominating Set gamma = ", g, "time = ", end_time - start_time)
    epoch, min_conv_cand = best_epoch(filepath, max_epochs)
    if min_conv_cand <= min_conv:
        best_g = g
        
# Get final number of epochs
filepath = ("runs/DomSet/optimizer_g_" + str(best_g))
epoch, min_conv_cand = best_epoch(filepath, max_epochs)
