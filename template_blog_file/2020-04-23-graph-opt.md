...
To do so, however, often requires solving combinatorial optimization problems over these graphs. **Combinatorial optimization** is a field in applied mathematics that explores the methods of finding some object or set of objects that is 'optimal' out of a finite set of such objects. This optimality is assessed by a cost function that maps each potential outcome to an associated cost, and minimizes that cost.
...
![alt text](./blog_images/blog_whitening/1d_non_gaussian.png "1-D Non-Gaussian")
